...
{
    [HttpPost("sessions")]
    public ActionResult<CreateChatSessionResponse> CreateSession([FromBody] CreateChatSessionRequest request)
    {
        var session = chatService.CreateSession(new ApplicantInfo(request.Name, request.Dni, request.Phone, request.Email));
        return Ok(new CreateChatSessionResponse(session.Id, "Sesión creada", chatService.HasActiveAdvisors()));
    }

    [Authorize]
    [HttpGet("pending")]
    public ActionResult GetPendingChats() => Ok(chatService.GetPendingChats());
    ...
    public async Task<IActionResult> TakeChat(Guid sessionId)
    {
        ...
        await hubContext.Clients.Group("advisors").SendAsync("chatUpdated", new { sessionId, status = chat.Status.ToString(), assignedAdvisorId = chat.AssignedAdvisorId });
        await hubContext.Clients.Group($"advisor-{advisorId}").SendAsync("assignmentNotification", new { sessionId, message = "Se te asignó una nueva solicitud." });
    ...
    public async Task<IActionResult> TransferChat(Guid sessionId, [FromBody] TransferChatRequest request)
    {
        ...
        await hubContext.Clients.Group("advisors").SendAsync("chatUpdated", new { sessionId, status = chat.Status.ToString(), assignedAdvisorId = chat.AssignedAdvisorId });
        await hubContext.Clients.Group($"advisor-{request.TargetAdvisorId}").SendAsync("assignmentNotification", new { sessionId, message = $"Se te asignó la solicitud {sessionId}." });
    ...
    public async Task<IActionResult> CloseChat(Guid sessionId, [FromBody] CloseChatRequest request)
    {
        ...
        var room = $"chat-{sessionId}";
        await hubContext.Clients.Group(room).SendAsync("chatClosed", new { sessionId, closedBy, reason = request.Reason });
    ...
    [Authorize]
    [HttpPost("advisors/status")]
    public IActionResult ToggleAdvisorStatus([FromBody] ToggleAdvisorStatusRequest request)
    {
        var advisorId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var advisorName = User.FindFirstValue(ClaimTypes.Name) ?? advisorId;
    ...
    [Authorize]
    [HttpGet("advisors")]
    public IActionResult GetAdvisors() => Ok(chatService.GetAdvisors());
}
