...
public class ChatHub(IAdvisorConnectionManager connectionManager, IAdvisorRepository advisorRepository) : Hub
{
    public override async Task OnConnectedAsync()
    {
        ...
        if (role == "advisor")
        {
            ...
            connectionManager.AddConnection(advisorId, Context.ConnectionId);
            advisorRepository.EnsureAdvisorExists(advisorId, advisorName);
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        {
            var advisorId = Context.User?.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown";
            connectionManager.RemoveConnection(advisorId, Context.ConnectionId);
    ...
    {
        await Clients.Group($"chat-{sessionId}").SendAsync("typingChanged", new { sessionId, senderType, isTyping });
    }
}
