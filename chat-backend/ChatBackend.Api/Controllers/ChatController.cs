...
public class ChatController(IChatService chatService, IHubContext<ChatHub> hubContext) : ControllerBase
{
    ...
    {
        var applicant = new ApplicantInfo(request.Name, request.Dni, request.Phone, request.Email);
        var session = chatService.CreateSession(applicant);
    ...
    [Authorize]
    [HttpGet("advisors")]
    public ActionResult GetAdvisors() => Ok(chatService.GetAdvisors());
    ...
    public ActionResult GetSession(Guid sessionId)
    {
